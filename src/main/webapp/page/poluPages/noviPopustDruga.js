ustanoveApp.controller('noviPopustDrugaController', ['$scope', '$http', '$state','$stateParams', function ($scope, $http, $state, $stateParams) {
    $scope.arejRedova=[];
    $scope.arejSedista=[];
    var redTr,sedisteTr,idTr;
    $scope.ima=false;
    $scope.toRez3 = function() {
        $state.go("noviPopustTreca");
    }
    ...
    $scope.toRez3 = function(id) {
        $state.go("noviPopustTreca", {id: id});
    }
}]);