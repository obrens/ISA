ustanoveApp.controller('promeniUloguController', ['$scope', '$http', '$state', '$window', function ($scope, $http, $state, $window) {
    ...
    $scope.nadjiUloge = function () {
        $scope.ulogeIzabr = [];
        $scope.uloge = $scope.sacuvane;
        console.log($scope.uloge);
        console.log($scope.sacuvane);
    ...
    $scope.izmeni = function () {
        $http.get('/api/korisnik/' + $scope.izabranKorisnik.id).success(function (data1) {
